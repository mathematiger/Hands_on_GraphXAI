...
if num_layers_gnn == 4:
    model = GNN_GCN_4(in_channels=2, hidden_channels=32, out_channels=data.num_classes, num_layers = num_layers_gnn)
    ...
    model = GNN_GCN_3(in_channels=2, hidden_channels=32, out_channels=data.num_classes, num_layers = num_layers_gnn)
    optimizer = torch.optim.Adam(model.parameters(), lr = 0.001, weight_decay = 0.001)
    criterion = torch.nn.CrossEntropyLoss()
    if new_dataset == True:
        for _ in range(500):
    ...
    model = GNN_GCN_2(in_channels=2, hidden_channels=32, out_channels=data.num_classes, num_layers = num_layers_gnn)
    optimizer = torch.optim.Adam(model.parameters(), lr = 0.001, weight_decay = 0.001)
    criterion = torch.nn.CrossEntropyLoss()
    if new_dataset == True:
        for _ in range(500):
...
model = GIN(input_feat = 2, hidden_channels = 32, classes = data.num_classes)
for _ in range(500):
    loss = train(model, optimizer, criterion, data)


node_idx = choose_xnode(data, label_to_explain)
