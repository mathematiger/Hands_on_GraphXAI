...
def create_dataset(max_dist):
    ...
    N = len(g.nodes())
    infected_nodes = rnd.sample(g.nodes(), 50)
...
class GNN(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers):
        super().__init__()
        self.conv1 = SAGEConv((-1, -1), hidden_channels, dropout = 0.1)
        self.conv2 = SAGEConv((-1, -1), hidden_channels, dropout = 0.5)
        self.conv3 = SAGEConv((-1, -1), hidden_channels, dropout = 0.3)
...
def choose_xnode(data):
    boolean_tensor = data.test_mask
...
def get_exp_method(method, model, criterion, pred_class, data, layers_of_gnn, node_idx):  
    ...
    if method=='gnnex':
        ...
        exp_method=PGExplainer(model, emb_layer_name = 'conv'+str(num_layers), max_epochs = 10, lr = 0.1)
        forward_kwargs={'node_idx': node_idx,
                        'x': data.x.to(device),
                        'edge_index': data.edge_index.to(device),
                        'label': pred_class}
